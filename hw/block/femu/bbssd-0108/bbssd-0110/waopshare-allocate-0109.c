...
static void ssd_init_usertime(struct ssd *ssd) {
    ...
    for (int user_id = 0; user_id < ssd->users_count; user_id++) {
        ...
        ssd->users[user_id].re_com_time = (int) ssd->users[user_id].tt_time * COM_PER_TIME;
...
static struct line *select_victim_line_compaction(struct ssd *ssd, int user_id)
{
    ...
    for(int i = 0; i < spp->tt_lines; i++)
    {
        ...
        efficiency_line = tt_pgs * zombietime / line->vpc;
        ...
        case REST:
            if (time_s - ssd->last_measure_time_s >= REST_TIME) {
                ...
                for (int user_id = 0; user_id < ssd->users_count; user_id++)
                {
                    ...
                    my_log(ssd->fp_info, "REST TIME DONE... [%d]WAF_BEGIN = %lf\n", user_id, ssd->users[user_id].WAF_begin);
                }
        ...
        case MEASURE:
            if (time_s - ssd->last_measure_time_s >= MEASURE_TIME) {
                ...
                for (int user_id = 0; user_id < ssd->users_count; user_id++)
                {
                    ...
                    my_log(ssd->fp_info, "MEASURE TIME DONE... [%d]WAF_END = %lf, deltax = %lf\n", user_id, ssd->users[user_id].WAF_end, ssd->users[user_id].delta_x);
                }
...
static struct line *select_victim_line_vssd(struct ssd *ssd, bool force, int user_id)
{
    ...
    if (force) {
        ...
        my_log(ssd->fp, "Now LOW GC Line[%d] has %d valid page. %d invalid page.\n",  victim_line->id, victim_line->vpc, victim_line->ipc);
    }
