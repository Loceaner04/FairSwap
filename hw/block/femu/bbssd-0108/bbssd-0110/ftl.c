...
static void printf_info(struct ssd *ssd) {
    ...
    if(time_s > ssd->last_print_time_s && ssd->test_begin) {
        ...
        for (int user_id = 0; user_id < ssd->users_count; user_id++)
        {
            if (ssd->users[user_id].re_com_time > 0 && should_compaction(ssd, user_id) && ssd->users[user_id].lm.victim_line_cnt != 0 && ssd->ismeasure == MEASURE) {
                ssd->isgc = true;
            ...
            if (time_s - ssd->last_measure_time_s >= REST_TIME) {
                ...
                for (int user_id = 0; user_id < ssd->users_count; user_id++)
                {
                    if (ssd->users[user_id].WAF_write_count == 0) {
                        ...
                        ssd->users[user_id].WAF_begin = (double) (ssd->users[user_id].WAF_compaction_count + ssd->users[user_id].WAF_write_count + ssd->users[user_id].WAF_gc_count) / (double) ssd->users[user_id].WAF_write_count;
            ...
            if (time_s - ssd->last_measure_time_s >= REST_TIME) {
                ...
                for (int user_id = 0; user_id < ssd->users_count; user_id++)
                {
                    if (ssd->users[user_id].WAF_write_count == 0) {
                        ...
                        ssd->users[user_id].WAF_end = (double) (ssd->users[user_id].WAF_compaction_count + ssd->users[user_id].WAF_write_count + ssd->users[user_id].WAF_gc_count) / (double) ssd->users[user_id].WAF_write_count;
