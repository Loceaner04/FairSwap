...
#define HOT 1
#define WARM 2
#define GLOBAL_COLD 5
#define NUM_DATA 3
#define REST_TIME 1
#define MEASURE_TIME 18
#define ALLOCATE_TIME 10 //分配时间
#define BEFORE_MEASURE_TIME 1
#define START_TIME 70
#define LIMIT_STORAGE_BY_TOTAL 0.05

#define BEFORE_MEASURE -3
#define BEFORE_BEGIN -2
#define NONE -1
#define REST_BEGIN 0
#define MEASURE 1
#define ALLOCATE 2
#define REST_END 3
#define COM_PER_TIME 0.15
#define SEQQUEUE_MAXSIZE 10
#define LINE_SORT_MAXSIZE 200
...
typedef struct sort_line_struct {
    int user_id;
    int vpc;
} sort_line_struct;
...
struct users {
    ...
    int64_t re_com_time;       //可以做compatcion的时间
...
struct ssd {
    ...
    struct ssd_channel *ch;
    struct line *lines;
    struct sort_line_struct *sort_lines;
    struct ppa *maptbl; /* page level mapping table */
    uint64_t *rmap;     /* reverse mapptbl, assume it's stored in OOB */
    // struct write_pointer wp;
    ...
    int bug_com;
    int bug_gc;
    int64_t tt_time_global;                    
    int64_t re_time_global;
    int64_t re_com_time_global;
}; 
