...
static void printf_info(struct ssd *ssd) {
    ...
    if(time_s > ssd->last_print_time_s && ssd->test_begin) {
        ...
        if (ssd->read_req_persec+ssd->write_req_persec+ssd->gc_read_req_persec+ssd->gc_write_req_persec+ssd->gc_erase_req_persec+ssd->com_read_req_persec+ssd->com_write_req_persec+ssd->com_erase_req_persec!=0) {
            ...
            for(int user_id = 0; user_id < ssd->users_count; user_id++) {
                my_log(ssd->fp_info, "\tuser_id:%d, IOs:(%d,%d), GC_IOs:(%d,%d,%d), COMPACTION_IOs:(%d,%d,%d), re_time = %ld\n", user_id, ssd->users[user_id].read_req_persec,ssd->users[user_id].write_req_persec, ssd->users[user_id].gc_read_req_persec, ssd->users[user_id].gc_write_req_persec, ssd->users[user_id].gc_erase_req_persec, ssd->users[user_id].com_read_req_persec, ssd->users[user_id].com_write_req_persec, ssd->users[user_id].com_erase_req_persec, ssd->users[user_id].re_time);
            }
        }
        for(int user_id = 0; user_id < ssd->users_count; user_id++) {
        ...
        for (int user_id = 0; user_id < ssd->users_count; user_id++)
        {
            if (should_compaction(ssd, user_id) && ssd->users[user_id].lm.victim_line_cnt != 0 && ssd->ismeasure == MEASURE) {
                ssd->isgc = true;
...
static struct line *select_victim_line_compaction(struct ssd *ssd, int user_id)
{
    ...
    if (victim_line->ipc < ssd->sp.pgs_per_line / 8 && !force) {
        return NULL;
    }
